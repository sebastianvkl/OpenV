...
## D039 — Materials and version-bound simulation scenes

**Decision (user steering, 2026-09-08):** Keep the exported CAD mesh authoritative for displayed part geometry. Procedural printed, carbon and plywood materials, studio lighting and component callouts improve readability without altering engineering dimensions. Wire routes and ground/bench scenery are explicitly illustrative; purchased parts remain envelopes.

Add a domain-owned VLM visualization artifact after each verification. It links the exact design, baseline, trim evidence ID/output hash, source revision, inputs and units. Reject mismatched inputs, corrupted trim, inadmissible trim, nonfinite output or excessive panel boundary residual. The artifact contains actual AeroSandbox lifting-surface streamlines and panel normal loads, and exposes disagreement with the AeroBuildup trim model. It supplies no PASS metrics and never replaces the verification engine. Include it in the hash-manifest package and link it from the Dalus-attached verification snapshot. Coarse inviscid panels do not establish CFD, stall, turbulence or physical flight.

Airflow, structural-load and flight views share the selected version's recorded evidence. Speed, altitude and load presets are drafts until the existing canonical mission-amendment pipeline reruns CAD and verification through Dalus MCP. No new simulator service, queue or editable engineering store is introduced. Completed cases are comparable only when parameters, components, contracts and all other scenario inputs match.

## D040 — Animated world as presentation, not flight evidence

**Decision (user steering, 2026-09-08):** Put the exact generated aircraft into a procedural 3D valley, coast or mountain scene. Animate a prescribed 90 m radius circuit at the recorded airspeed, with 35 m scenic height, cosmetic propeller rotation and a cinematic bank. Chase, wing and survey cameras, pause/restart and playback rate are viewer controls. Respect reduced-motion preferences on initial playback.

The circuit, bank, terrain and scenic height are illustrative; the cinematic height is not the mission's atmospheric altitude. This is not a six-degree-of-freedom flight solver, controller, trajectory verification or a landing analysis. Keep actual recorded PASS/FAIL/UNKNOWN and operating conditions visible. Selecting scenery never changes evidence or engineering state. Selecting a different computed operating case changes the version-bound analysis and recorded animation speed. No backend methods or released artifacts change for this rendering feature.

## D041 — Read-only component inspection and storefront links

**Decision (user steering, 2026-09-08):** Clicking CAD parts, or selecting a hidden part from the accessible component picker, displays the exact frozen component catalog: manufacturer, part number, property values, units, quality and recorded sources. Custom parts expose CAD bounds, modeled mass, stock dimensions and their version-matched STEP/STL download. Selecting a part pauses viewer motion for inspection.

Keep three verified storefront URLs as presentation-only metadata keyed by manufacturer and exact part number (EMAX EMX-MT-0409, Tattu TAA13003S75X6, Hobbywing 30205200). These links do not update specifications, price, stock, evidence or historical artifacts. Unselected servos, receiver and propeller have no invented product link. Render only HTTP(S) source links; distinguish recorded datasheets/drawings from purchase navigation. A sourced property does not establish installed-system verification.

## D042 — One-system installation verification extension

**Decision (user-authorized two-hour extension, 2026-09-08):** Concentrate new work on one motor-glider. Add manufacturer-selected servo, receiver and propeller to new immutable system baselines, and a versioned installation interface with explicit parts, placements, assembly order and remaining unknowns. Historical source catalogs and contracts are preserved. CAD consumes this installation revision; regeneration packages carry it.

Use external solid geometry operations to evaluate purchased-part interference, a conservative continuous propeller swept envelope, and straight-line insertion swept volumes under a declared assembly sequence. These are individually scoped requirements, not a replacement for the full assembly/airframe/physical gates. Publish checked pairs, exclusions with their interface reason, clearances, collisions and source inputs. Add bounded installation-position parameters so Astra can repair actual geometry failures while contracts and sourced properties stay protected. Voltage compatibility and channel capacity are independent deterministic checks; missing servo current/installed thrust information remains UNKNOWN.

The first solid check exposed servo bodies crossing segment boundaries, motor/pylon intersection and an obstructed ESC insertion path. Revised ports span every affected segment; the pylon meets the firewall ahead of the motor, and internal positions are explicit parameters. New CAD includes open servo cradles, slotted trays with pod-conforming support feet, two pod seam collars and center spar sockets. These are design definitions, not joint/retention load evidence.

New stock alternatives are Easy Composites CFT-WF-10-8-1 and CFT-WF-12-10-1. Matched CAD sections use sourced linear mass and 64 GPa axial modulus; the bending-strength allowable remains explicitly assumed. A procurement contract rejects unmatched diameters/walls or cuts exceeding 1 m stock minus 3 mm kerf. The APC mass source is 0.46 oz, converted and rounded to 13.04 g; excessive decimal digits from exact unit conversion exceeded Dalus numeric storage precision. The adapter's exact read-back comparison remains intact.


## D043 — One persistent Dalus model for the aircraft

**Decision (explicit user steering, 2026-09-08):** Reuse one Dalus model for this hardware system across mission edits, design changes and new pipeline runs. This supersedes the per-run Dalus-model isolation in D026/D030 and the public-runtime plan. Run artifacts remain immutable and separately addressed. The shared model represents the latest admitted candidate, while existing test-case runs, engineering experiments and pre-update snapshots preserve history. Historical website results describe their captured version, not the current Dalus verdict.

Persist model/element IDs in a restricted mapping file (`OPENV_DALUS_MAPPING`, default `OPENV_AUTH_DIR/dalus-system.json`). Reuse requirement customer IDs, test cases, parts, parameters and interface routes; add only genuinely new engineering elements. Set old verdicts pending before changing inputs, read back exact canonical values/contracts and current evidence/statuses, and leave partial updates incomplete. Missing mapped elements or ambiguous IDs block updates rather than silently duplicating them. Deprecated requirements remain in history with no current PASS. An OS file lock enforces one local writer; this is not a distributed transaction guarantee. Existing models are preserved; the reference model from run-7342013fdee4 is the deployed system model. Engineering access continues through MCP only.

## D044 — Computed point-mass response playback

**Decision (implementation of the animated-simulation request, 2026-09-08):** Extend each new aircraft analysis with a version-bound `flight-response.json`. AeroSandbox AeroBuildup evaluates lift/drag from the candidate geometry at recorded trim alpha/elevator over 8–22 m/s. AeroSandbox three-dimensional point-mass equations and SciPy RK45 integrate trim hold, power loss at 5 s, a prescribed 30° bank ramp and uniform 4 m/s crosswind. Publish force tables, trajectory samples, assumptions, trim evidence ID/hash, source revision, integration settings and a time-step refinement comparison. Diagnostic results supply no verification metrics and cannot grant PASS.

Ideal attitude tracking and prescribed thrust are assumptions; this is not a controller, installed propulsion measurement, gust model, stall model or six-degree-of-freedom rigid-body flight test. Start at 60 m above a flat reference plane, independently of atmospheric altitude. Stop playback at the computed endpoint or the first ground/speed/flight-path model limit. The browser interpolates recorded samples and shows current height, speed and thrust. Cosmetic scenery/RPM remain labeled; historical runs without the artifact retain their explicitly prescribed circuit. This extends D040 without changing historical evidence or closing physical-flight requirements.


## D045 — Read-only browser STEP inspection

**Decision (user request, 2026-09-08):** Assemble defaults to a STEP CAD tab alongside the existing assembly guide. A dedicated browser worker loads the selected design's actual assembly STEP using occt-import-js 0.0.23 and tessellates it in meters at 0.15 mm linear deflection. Assembly hierarchy names identify parts whose underlying solids have generic names. Reject ambiguous/missing identities, malformed meshes and bounds differing by more than 1 mm from the captured assembly before attaching frozen component metadata. This is a display admission check, never engineering evidence or PASS. Only load after the selected version's captured geometry arrives; cancel workers when switching away and cache two imported versions in memory. Failed imports leave an explicitly labeled captured-mesh preview and a download/retry option.
