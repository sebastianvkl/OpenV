...
- [x] Commit `dd13e47` deployed on Vercel Hobby. With AWS stopped, all 20 run snapshots matched the originals, eight representative STEP/package/evidence hashes matched, all four write methods returned 403, and the page/video remained available. Production browser checks passed the two-collision repair, computed flight, actual STEP import and mobile layout, with no JavaScript errors, POST requests or continued snapshot polling.
- [x] Terminated the dedicated EC2 instance and confirmed its disk was deleted, its automatic public IP was released, and no matching Elastic IP or volume snapshots remained. Removed temporary SSH access. Preserved the verified private host backup locally; the public demo now needs no paid backend or model credentials. Existing accrued provider charges and free-hosting usage caps still apply.
